...
@search_blueprint.command("anime", "Search for information on a given Anime.", guild_ids=[629317680481959946, 675647130647658527])
async def search_anime(app: CommandHandler, interaction: Interaction, query: str):
    ...
    embed = Embed(
        color=EMBED_COLOUR,
        thumbnail=EmbedImage(url=random.choice(RANDOM_THUMBNAILS))
    )
    embed.set_author(
...
async def run_query(app: CommandHandler, query: OptionData = None):
    ...
    return [CompletedOption(name=hit.get('title_english') or hit['title'], value=hit['id']) for hit in hits]
